...
document.writeln('<SCRIPT TYPE="text/javascript" LANGUAGE="JavaScript" SRC="http://images.mmorpg.com/scripts/wddx.js?cache=04072010BK"></SCRIPT >');
...
function http(verb,url,rm,qry,mute) {
    //reference our arguments
	var mute	= true;//(!mute) ? true : mute;
	var callback = rm;
				...
				case 4:
					...
					if ( req.status == 200 ){// only if "OK"
						try{
							...
							if(!mute) jsmx.onError('Parsing Error: The value returned could not be evaluated.');
							success = false;
						}
						if(success) callback( rObj );
					}else{ 
						if(!mute) jsmx.onError("There was a problem retrieving the data:\n" + req.statusText);
					}
					break;
				}
			}
