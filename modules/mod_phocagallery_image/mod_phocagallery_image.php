	...
	foreach($imagesArray as $valueImages){
		...
		switch($module_type) {
			...
			default:
				...
				$attrTitle = $valueImages->title;
				if ($tmpl['detail_window'] == 8) {
					if ($valueImages->description != '') {
						$attrTitle = $valueImages->title . ' - '.strip_tags($valueImages->description);
					}
				}
				...
				$output[$i] .= '<div class="pg-cv-box-img-mod-ri pg-box1" >'. "\n"
					...
					.'<div class="pg-box3">' . "\n"
					.'' . "\n"
					.'<a class="'.$button->methodname.'" title="'.$attrTitle.'" href="'. JRoute::_($valueImages->link).'"'; 
