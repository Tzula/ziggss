...
class ContentModelArticle extends JModelItem
{
	...
	public function getArticlesLatest(){
		...
		if (isset($article->catid)){
			$sql = "select * from #__content as a where a.state=1 and a.catid= " . $article->catid  . " order by a.created desc limit 8";
		}else{
			$sql = "select * from #__content as a where a.state=1  order by a.created desc limit 8";
		}
