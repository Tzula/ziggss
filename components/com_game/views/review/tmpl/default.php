...
?>
	...
	<div class="panelOuterBody">
		...
		<div id="" class="panelBody ">
				...
				<div class="gvconblock">
						...
						<div class="reviews"> 
						<div id="sectionHeader"><span><h1>Review:<?php echo $this->new->title . ' ';?></h1>
					</span></div>
	
	<div class="gvpricon">
		
	<div class="newsItem">
			<div class="commonContent noheader notitle">
	...
	<span style="display:none">Leave this field empty <input id="fpCFFC8009-3048-C53A-9498C39BECB2CC9A" type="text" name="formfield1234567894" value=""></span>
							...
							<br/>
				<br/>		
				<div class="features" style="margin: 0 10px 0px 10px;"><h2>Other News Of This Game</h2>
					<ul class="spotlight">
						<?php if (!empty($this->reviewsOfThisGame)) : ?>
						<?php $count = count($this->reviewsOfThisGame);?>
						<?php foreach ($this->reviewsOfThisGame as $i => $row) :
				...
				<div class="features" style="margin: 0 10px 0px 10px;"><h2>Other News Of This Category</h2>
					<ul class="spotlight">
						<?php if (!empty($this->reviewsOfThisCategory)) : ?>
						<?php $count = count($this->reviewsOfThisCategory);?>
						<?php foreach ($this->reviewsOfThisCategory as $i => $row) :
							$link = JRoute::_('index.php?option=com_game&view=review&id=' . $row->gameid . '&reviewid=' . $row->id);
							$img = json_decode($row->images)->image_intro;
							if ($img == ''){
								$img = 'media/other/generic.png';
