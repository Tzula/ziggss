 ...
 * @since  0.0.1
 */
class GameModelReview extends JModelItem
{
	/**
	...
	public function getNew()
	{
		...
		$reviewid = $jinput->get('reviewid');
		$query->where('a.id =' . $reviewid);
	...
	public function getReviewsOfThisGame(){
		// Create a new query object.
		$db = $this->getDbo();
		$query = $db->getQuery(true);
		$user = JFactory::getUser();
		...
		$sql = "select * from #__content as a where a.state=1 and a.catid =22 and a.gameid=$gameid  order by a.created desc limit 4";
	...
	public function getReviewsOfThisCategory(){
		// Create a new query object.
		$db = $this->getDbo();
		$query = $db->getQuery(true);
		$user = JFactory::getUser();
		...
		$sql = "select * from #__content as a where a.state=1 and a.catid =22 and a.gameid in (select b.id from #__game as b  where b.genres=$gameCid )  order by a.created desc limit 4";
