 ...
 * @since  0.0.1
 */
class GameModelVideo extends JModelItem
{
	/**
	...
	public function getVideo()
	{
		...
		$videoid = $jinput->get('videoid');
		$query->where('a.id =' . $videoid);
	...
	public function getVideosOfThisGame(){
		// Create a new query object.
		$db = $this->getDbo();
		$query = $db->getQuery(true);
		$user = JFactory::getUser();
		...
		$sql = "select * from #__content as a where a.state=1 and (a.catid = 19 or a.catid= 58 or a.catid=59 or a.catid=61 or a.catid=62) and a.gameid=$gameid  order by a.created desc limit 4";
	...
	public function getVideosOfThisCategory(){
		// Create a new query object.
		$db = $this->getDbo();
		$query = $db->getQuery(true);
		$user = JFactory::getUser();
		...
		$sql = "select * from #__content as a where a.state=1 and (a.catid = 19 or a.catid= 58 or a.catid=59 or a.catid=61 or a.catid=62) and a.gameid in (select b.id from #__game as b  where b.genres=$gameCid )  order by a.created desc limit 4";
